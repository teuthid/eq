...
bool EasyButton::read() {
  ...
  if (read_started_ms - _last_change < _db_time) {
    ...
    _current_state =
        pinVal; // assign new state as current state from pin's value.
    _changed =
        (_current_state != _last_state); // report state change if current state
                                         // vary from last state.
  ...
  if (!_current_state && _changed) {
    ...
    if (!_was_btn_held) {
      ...
      if (_short_press_count == _press_sequences &&
          _press_sequence_duration >= (read_started_ms - _first_press_time)) {
      ...
      else if (_press_sequence_duration <=
               (read_started_ms - _first_press_time)) {
        _short_press_count = 0;
        _first_press_time = 0;
      }
    }
  ...
  // button is not released.
  else if (_current_state && _time - _last_change >= _held_threshold &&
           mPressedForCallback) {
    ...
    _short_press_count = 0;
    _first_press_time = 0;
    // call the callback function for a long press event if it exist and if it
